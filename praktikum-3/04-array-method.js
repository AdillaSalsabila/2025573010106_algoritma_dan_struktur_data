...
console.log(`Total: Rp${totalInventaris.toLocaleString('id-ID')}`);

console.log("\n--- Daftar Status Produk ---");
produk.forEach(p => {
    const hargaFormat = p.harga.toLocaleString('id-ID');
    if (p.stok > 0) {
        console.log(`[TERSEDIA] ${p.nama} - Rp${hargaFormat} (${p.stok} unit)`);
    } else {
        console.log(`[HABIS] ${p.nama} - Rp${hargaFormat}`);
    }
});
